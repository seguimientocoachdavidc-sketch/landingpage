...
function listarDiasGuardados(token: string): string[] {
  ...
  for (let i = 0; i < localStorage.length; i++) {
    ...
    if (key?.startsWith(`macros_v2_${token}_`)) {
      const fecha = key.replace(`macros_v2_${token}_`, "")
      keys.push(fecha)
    }
  }
...
export default function MacrosPage() {
  ...
  const [vistaInicio, setVistaInicio]   = useState(true) // true = pantalla de selección de fecha
  ...
  const abrirDia = (tok: string, fecha: string, nombre: string) => {
    ...
    setVistaActiva("agregar")
    setDiasGuardados(listarDiasGuardados(tok))
  }
  ...
  const agregarEntrada = () => {
    ...
    const nuevaData: DiaData = { ...diaData, entradas: [...diaData.entradas, entrada] }
    setDiaData(nuevaData)
    guardarLocal(nuevaData)
  ...
  /* ── Nuevo día tras cerrar ── */
  const iniciarNuevoDia = () => {
    setDiaData(null)
    setVistaInicio(true)
    setFechaSeleccionada("")
    if (token) setDiasGuardados(listarDiasGuardados(token))
  }
  ...
  if (vistaInicio) return (
    <div style={{ background:"#000", color:"#fff", fontFamily:"'Barlow',sans-serif",
      ...
      <div style={{ maxWidth:560, margin:"0 auto", padding:"60px 24px" }}>
        ...
        {/* Logo */}
        <div className="bc" style={{ fontSize:16, fontWeight:900, marginBottom:48 }}>
          COACH<span style={{ color:R }}>.</span>DAVID
          <span className="b" style={{ fontSize:12, color:"rgba(255,255,255,0.35)",
            letterSpacing:"0.2em", textTransform:"uppercase", marginLeft:12 }}>
            Macros
          </span>
        </div>
        ...
        <div style={{ marginBottom:36 }}>
          ...
          <p className="b" style={{ fontSize:14, color:"rgba(255,255,255,0.4)",
            ...
            Selecciona la fecha del día que quieres registrar o continuar.
          </p>
        </div>
        ...
        <div style={{ marginBottom:20 }}>
          ...
          <input type="date" value={fechaSeleccionada}
            ...
            style={{ width:"100%", padding:"14px 16px",
              ...
              border:`1px solid ${fechaSeleccionada?R:"rgba(255,255,255,0.12)"}`,
        ...
        <button
          ...
          onClick={() => abrirDia(token, fechaSeleccionada, nombreUsuario)}
          style={{ width:"100%", padding:"16px",
            background:fechaSeleccionada?R:"rgba(232,0,13,0.25)",
            border:"none", color:fechaSeleccionada?"#fff":"rgba(255,255,255,0.3)",
            fontFamily:"'Barlow Condensed',Impact,sans-serif", fontSize:15,
          ...
          {fechaSeleccionada
            ? `Abrir ${formatFecha(fechaSeleccionada)} →`
            : "Selecciona una fecha"}
        </button>

        {/* Días anteriores guardados */}
          ...
          <div>
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:6 }}>
              {diasGuardados.map(fecha => {
                const key  = getStorageKey(token, fecha)
    ...
    <div style={{ background:"#000", color:"#fff", fontFamily:"'Barlow',sans-serif",
      ...
      <header style={{ position:"fixed", top:2, left:0, right:0, zIndex:100,
        ...
        <div style={{ maxWidth:960, margin:"0 auto",
          ...
          <div style={{ display:"flex", alignItems:"center", gap:12 }}>
            {/* Botón volver */}
            <button onClick={iniciarNuevoDia}
            ...
            <a href="/" className="bc" style={{ fontSize:16, fontWeight:900,
          ...
          <div style={{ display:"flex", alignItems:"center", gap:8 }}>
            <div style={{ width:6, height:6,
              background:diaData?.cerrado?"#22c55e":R,
      ...
      <div style={{ position:"fixed", top:56, left:0, right:0, zIndex:99,
        ...
        <div style={{ maxWidth:960, margin:"0 auto", padding:"0 16px", display:"flex" }}>
          {[
            { key:"agregar", label:"Registrar comida" },
            { key:"resumen", label:`Resumen del día (${diaData?.entradas.length ?? 0})` },
          ].map(tab => (
            <button key={tab.key} onClick={() => setVistaActiva(tab.key as any)}
              ...
              style={{ padding:"12px 20px", fontSize:13, fontWeight:700,
                ...
                color:vistaActiva===tab.key?"#fff":"rgba(255,255,255,0.35)",
                borderBottom:`2px solid ${vistaActiva===tab.key?R:"transparent"}`,
      ...
      <div style={{ maxWidth:960, margin:"0 auto",
        ...
        <div style={{ marginBottom:28, display:"flex", alignItems:"flex-start",
          ...
          <div>
            <div className="bc" style={{ fontSize:11, color:R,
              ...
              {diaData ? formatFecha(diaData.fecha) : ""}
            </div>
            <div className="bc" style={{ fontSize:isMobile?28:36, fontWeight:900,
          ...
          {diaData?.cerrado && (
            <div style={{ padding:"8px 16px",
              ...
              <span className="bc" style={{ fontSize:12, color:"#22c55e",
        ...
        {vistaActiva === "agregar" && (
          <div style={{ animation:"fadeUp 0.3s ease" }}>
            {diaData?.cerrado ? (
              /* Día cerrado — opción de crear nuevo día */
              <div style={{ padding:"32px 24px",
                ...
                <p className="b" style={{ fontSize:14, color:"rgba(255,255,255,0.45)",
                  marginBottom:24, lineHeight:1.6, fontWeight:300 }}>
                  El resumen fue enviado a Coach David.
                  ¿Quieres registrar otro día?
                </p>
                <button onClick={iniciarNuevoDia}
                  style={{ padding:"14px 32px", background:R, border:"none",
              ...
              <>
                  ...
                  <div style={{ display:"flex", flexWrap:"wrap", gap:8 }}>
                    {[1,2,3,4,5,6].map(n => (
                      <button key={n} onClick={() => setComidaNum(n)}
                        ...
                        Comida {n}
                        {(diaData?.entradas.filter(e=>e.comida===n).length ?? 0) > 0 && (
                    ...
                    <div style={{ position:"absolute", top:"100%", left:0, right:0,
                      ...
                      {resultados.map(a => (
                        <button key={a.id}
                          ...
                          <div>
                            <div className="b" style={{ fontSize:14, color:"#fff",
                              fontWeight:400 }}>{a.nombre}</div>
                ...
                {/* Entradas comida actual */}
                {(diaData?.entradas.filter(e=>e.comida===comidaNum).length ?? 0) > 0 && (
        ...
        {vistaActiva === "resumen" && (
          <div style={{ animation:"fadeUp 0.3s ease" }}>
            {(diaData?.entradas.length ?? 0) === 0 ? (
              <div style={{ textAlign:"center", padding:"60px 20px",
                color:"rgba(255,255,255,0.3)" }}>
                <div className="bc" style={{ fontSize:48, marginBottom:12,
                  opacity:0.3 }}>◎</div>
                ...
                {!diaData?.cerrado && (
                  <div style={{ marginTop:32, padding:"24px",
                    ...
                    <p className="b" style={{ fontSize:14,
                      ...
                      Al cerrar el día, el resumen completo se envía automáticamente
                      a Coach David. Después podrás registrar otro día.
                    </p>
        ...
        {toast && (
          <div style={{ position:"fixed", bottom:24, left:"50%",
            ...
            <span style={{ color:toast.tipo==="ok"?"#22c55e":R,
    ...
    <div className="entrada-row" style={{ display:"flex", alignItems:"center",
      ...
      <button onClick={onEliminar}
        style={{ background:"none", border:"none", color:"rgba(255,255,255,0.2)",
          cursor:"pointer", padding:"4px 8px", fontSize:16,
  ...
  return (
    <button onClick={onClick} disabled={disabled}
      ...
      style={{ width:"100%", padding:"16px", border:"none",
        ...
        boxShadow:disabled||hover?"none":`0 0 30px ${R}30`,
...
function CerrarDiaBtn({ onClick, enviando, cerrado }: {
  ...
  if (cerrado) return (
    <div style={{ textAlign:"center", padding:"14px",
      ...
      <span className="bc" style={{ fontSize:14, color:"#22c55e",
        letterSpacing:"0.2em", textTransform:"uppercase" }}>
  ...
  return (
    <button onClick={onClick} disabled={enviando}
      onMouseEnter={()=>setHover(true)} onMouseLeave={()=>setHover(false)}
      style={{ width:"100%", padding:"16px", border:"none",
        background:enviando?"rgba(34,197,94,0.3)":hover?"#fff":"#22c55e",
  ...
  return (
    <div style={{ background:"#000", minHeight:"100vh", display:"flex",
      ...
      <div style={{ fontFamily:"'Barlow Condensed',Impact,sans-serif",
        fontSize:60, fontWeight:900, color:"#E8000D", marginBottom:16 }}>🔒</div>
      <h1 style={{ fontFamily:"'Barlow Condensed',Impact,sans-serif",
        fontSize:36, fontWeight:900, textTransform:"uppercase", marginBottom:12 }}>
        Acceso restringido
      </h1>
      <p style={{ fontSize:15, color:"rgba(255,255,255,0.45)", maxWidth:380,
        lineHeight:1.7, fontWeight:300 }}>
        Esta herramienta es exclusiva para clientes de Coach David
        con un plan de alimentación activo.
      </p>
