...
const TAG_COLORS: Record<string,string> = {
  "MUSCULACIÓN": R, "RUNNING": G, "CYCLING": B, "CORE": P, "BRICKS": O
}
...
/* ══ PÁGINA PRINCIPAL ══════════════════════════════════ */
export default function RivsPage() {
  ...
  const selDia = useCallback(async (dia: Dia) => {
    ...
    if (ant?.length) {
      ...
      if (rAnt) {
        ...
        rAnt.forEach(r=>{ if(!ma[r.ejercicio_id]) ma[r.ejercicio_id]={fecha:ant[0].fecha,data:[]}; ma[r.ejercicio_id].data.push({serie_num:r.serie_num,kg:r.kg,reps:r.reps}) })
        setAnts(ma)
      }
    }
  }, [token])
    ...
    <div style={{background:"#000",minHeight:"100vh",color:"#fff",
      ...
      <div style={{maxWidth:720,margin:"0 auto",padding:"20px 16px"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:6,marginBottom:24}}>
          {([
            {k:"semana", l:"📅 Semana",   c:"rgba(255,255,255,0.7)"},
            {k:"muscu",  l:"🏋️ Muscu",    c:R},
        ...
        {vista==="muscu" && (
          <div style={{animation:"fadeUp 0.3s ease"}}>
            ...
            {diaActivo && ejercicios.map((ej,idx)=>{
              ...
              const tieneImg = ej.video_url && !imgErr[ej.id]
              return (
        ...
        {/* ══ RUNNING ══ */}
        {/* 2 sesiones: Sesión 1 = Día 1 Easy Run, Sesión 2 = Día 6 Bricks parte carrera */}
        {vista==="running" && (
          <div style={{animation:"fadeUp 0.3s ease"}}>
            <div style={{fontSize:11,color:G,letterSpacing:"0.12em",textTransform:"uppercase",marginBottom:14}}>
              Running · {semanasRun.length} semanas · 2 sesiones por semana
            </div>
            ...
            {semanaRun && (
              <>
                {/* Sesión 1 — Día 1: Easy Run / Intervalos */}
                <SesionCard
                  color={G}
                  titulo="Sesión 1 · Día 1"
                  subtitulo="Easy Run / Intervalos"
                  icono="🏃"
                  descripcion={semanaRun.sesion_1_descripcion}
                  objetivo={semanaRun.sesion_1_objetivo_min}
                  existing={sesRun.find(s=>s.numero_sesion===1)??null}
                  abierta={abiertaRun===1}
                  onToggle={()=>setAbiertaRun(abiertaRun===1?null:1)}
                  onGuardar={(form,c)=>guardarRun(1,form,c)}
                  campos={[
                ...
                {/* Sesión 2 — Día 6: Bricks — parte carrera */}
                <SesionCard
                  color={O}
                  titulo="Sesión 2 · Día 6 — Bricks"
                  subtitulo="Parte de carrera tras ciclismo"
                  icono="🔥🏃"
                  descripcion={semanaRun.sesion_2_descripcion}
                  objetivo={semanaRun.sesion_2_objetivo}
                  existing={sesRun.find(s=>s.numero_sesion===2)??null}
                  abierta={abiertaRun===2}
                  onToggle={()=>setAbiertaRun(abiertaRun===2?null:2)}
                  onGuardar={(form,c)=>guardarRun(2,form,c)}
                  campos={[
                  ...
                  notaBricks="Esta sesión es la parte de carrera del Día 6 (Bricks). El ciclismo se registra en la pestaña Cycling."
                />
              </>
            )}
          </div>
        ...
        {/* ══ CYCLING ══ */}
        {/* 3 sesiones: Sesión 1 = Día 3 Continuo Z2, Sesión 2 = Día 5 Cycling Z2, Sesión 3 = Día 6 Bricks bici */}
        {vista==="cycling" && (
          <div style={{animation:"fadeUp 0.3s ease"}}>
            ...
            {semanaCyc && (
              <>
                {/* Sesión 1 — Día 3: Continuo Z2 */}
                <SesionCard
                  color={B}
                  titulo="Sesión 1 · Día 3"
                  subtitulo="Cycling Continuo Z2"
                  icono="🚴"
                  descripcion={semanaCyc.sesion_1_descripcion}
                  objetivo={semanaCyc.sesion_1_objetivo}
                  existing={sesCyc.find(s=>s.numero_sesion===1)??null}
                  abierta={abiertaCyc===1}
                  onToggle={()=>setAbiertaCyc(abiertaCyc===1?null:1)}
                  onGuardar={(form,c)=>guardarCyc(1,form,c)}
                  campos={[
                ...
                {/* Sesión 2 — Día 5: Cycling Z2 + transición */}
                <SesionCard
                  color={B}
                  titulo="Sesión 2 · Día 5"
                  subtitulo="Cycling Z2 + Carrera"
                  icono="🚴🏃"
                  descripcion={semanaCyc.sesion_2_descripcion}
                  objetivo={semanaCyc.sesion_2_objetivo}
                  existing={sesCyc.find(s=>s.numero_sesion===2)??null}
                  abierta={abiertaCyc===2}
                  onToggle={()=>setAbiertaCyc(abiertaCyc===2?null:2)}
                  onGuardar={(form,c)=>guardarCyc(2,form,c)}
                  campos={[
                    {k:"tiempo_min",l:"Tiempo (min)",p:"80",tipo:"number"},
                    {k:"distancia_km",l:"Distancia (km)",p:"30",tipo:"number"},
                    {k:"ritmo_promedio",l:"Vel. prom (km/h)",p:"22",tipo:"number"},
                    {k:"pulsaciones_prom",l:"Puls. prom",p:"138",tipo:"number"},
                  ]}
                  esCycling
                />

                {/* Sesión 3 — Día 6: Bricks — parte ciclismo */}
                <SesionCard
                  color={O}
                  titulo="Sesión 3 · Día 6 — Bricks"
                  subtitulo="Parte de ciclismo antes de la carrera"
                  icono="🔥🚴"
                  descripcion={semanaCyc.sesion_3_descripcion}
                  objetivo={semanaCyc.sesion_3_objetivo}
                  existing={sesCyc.find(s=>s.numero_sesion===3)??null}
                  abierta={abiertaCyc===3}
                  onToggle={()=>setAbiertaCyc(abiertaCyc===3?null:3)}
                  onGuardar={(form,c)=>guardarCyc(3,form,c)}
                  campos={[
                    {k:"tiempo_min",l:"Tiempo bici (min)",p:"80",tipo:"number"},
                    {k:"distancia_km",l:"Distancia (km)",p:"35",tipo:"number"},
                    {k:"ritmo_promedio",l:"Vel. prom (km/h)",p:"24",tipo:"number"},
                    {k:"pulsaciones_prom",l:"Puls. prom",p:"145",tipo:"number"},
                  ]}
                  esCycling
                  notaBricks="Esta sesión es la parte de ciclismo del Día 6 (Bricks). La carrera se registra en la pestaña Running."
                />
              </>
            )}
          </div>
        ...
        {vista==="core" && (
          <div style={{animation:"fadeUp 0.3s ease"}}>
            <div style={{fontSize:11,color:P,letterSpacing:"0.12em",textTransform:"uppercase",marginBottom:14}}>
              CORE · 3 opciones · Día 3 de la semana
            </div>
...
/* ══ COMPONENTE REUTILIZABLE: Tarjeta de sesión ════════ */
...
function SesionCard({color,titulo,subtitulo,icono,descripcion,objetivo,
  existing,abierta,onToggle,onGuardar,campos,esCycling,notaBricks}: SesionCardProps) {

  const initForm = () => {
    const f: Record<string,string> = {zona:"",nota:""}
    campos.forEach(c => { f[c.k] = existing?.[c.k]?.toString()??"" })
    f.zona = existing?.zona??"";  f.nota = existing?.nota??""
    return f
  }
  ...
  useEffect(() => { setForm(initForm()) }, [existing])

  const guardar = async (completar: boolean) => {
    setGuardando(true)
    await onGuardar(form, completar)
    setGuardando(false)
    setOk(completar?"✓ Sesión completada":"✓ Guardado")
        ...
        <div style={{display:"flex",alignItems:"center",gap:12}}>
          ...
          <div>
            <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontSize:15,fontWeight:800,
              textTransform:"uppercase",color:completada?color:"#fff"}}>
              {titulo}
            </div>
        ...
        <div style={{padding:"14px 16px"}}>
          ...
          {ok && (
            <div style={{marginTop:8,padding:"8px",textAlign:"center",
              ...
              color:color,fontSize:12}}>{ok}</div>
          )}
        </div>
      )}
    </div>
